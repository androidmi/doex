...
public class NetworkUtils {
    ...
    public static int getNetworkType(Context cxt) {
        ...
        if (netInfo != null) {
            ...
            if (DEBUG) Log.d(TAG, "network type = " + type + " : " + subType);
    ...
    public static String getIpAddress(Context ctx) {
        ...
        if (netInfo != null && netInfo.isConnected()) {
            if (DEBUG) {
                Log.d(TAG, "Active network found");
            }
            try {
                ...
                while (netIter.hasMoreElements()) {
                    ...
                    while (inetAddrIter.hasMoreElements()) {
                        ...
                        if (!inetAddr.isLoopbackAddress() && !TextUtils.isEmpty(ip)) {
                            if (DEBUG) {
                                Log.d(TAG, "Host name: " + inetAddr.getHostName()
                                        + ", IP: " + ip);
                            }
                            return ip;
                        }
        ...
        if (DEBUG) {
            Log.d(TAG, "Failed to get IP address");
        }
        return null;
    }
    ...
    public static HttpURLConnection openHttpURLConnection(Context ctx, String url) throws IOException {
        ...
        int proxyPort = -1;
        if(isNetworkMobile(ctx)) {
//            proxyAddr = AndroidNetProxyCompat.getHost(ctx, url);
//            proxyPort = AndroidNetProxyCompat.getPort(ctx, url);
        }
        if (proxyAddr != null && proxyPort != -1) {
            try {
                ...
                // The 'proxyAddr' or 'proxyPort' may be illegal.
                // Please refer bug [YHDS-1796].
            }
        }
